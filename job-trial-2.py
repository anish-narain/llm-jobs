...
def main(file_path, model_name, chunk_size, chunk_overlap, output_csv_file, progress_report_file, base_url, num_patients, num_gpu):
    ...
    llm = Ollama(model=model_name, base_url=base_url, num_gpu=num_gpu, callback_manager=CallbackManager([StreamingStdOutCallbackHandler()]))
...
if __name__ == "__main__":
    main(
        ...
        chunk_size=4096,
        chunk_overlap=100,
        output_csv_file='job-trial-1.csv',  # Change the output file name to a CSV file
        progress_report_file='job-trial-1.txt',  # Path to the progress report file
