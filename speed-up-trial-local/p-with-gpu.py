# WHAT I RAN TO GIVE ME THE 100 pneumonia results
import pandas as pd
import time
import random
import csv  
...
def main(file_path, model_name, chunk_size, chunk_overlap, output_csv_file, progress_report_file, num_patients, num_gpu):
    df = load_data(file_path)
    #start_index = select_random_start(len(df))
    start_index = 0
    prompt_template = create_prompt_template()
    llm = Ollama(model=model_name, num_gpu=num_gpu, callback_manager=CallbackManager([StreamingStdOutCallbackHandler()]))
...
if __name__ == "__main__":
    main(
        ...
        chunk_size=4096,
        chunk_overlap=100,
        output_csv_file='pneumonia-with-gpu.csv',  # Change the output file name to a CSV file
        progress_report_file='pneumonia-with-gpu.txt',  # Path to the progress report file
        num_patients = 10,
